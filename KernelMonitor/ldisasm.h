#pragma once
#include <cstddef>
// source: https://github.com/Nomade040/length-disassembler
// thanks @Nomade040!

using uint8_t = unsigned char;
namespace ldisasm
{
	enum class DisassmeblyMode {
		X86,
		X86_64
	};

	inline constexpr uint8_t LDISASM_R(uint8_t* b) { return *b >> 4; };
	inline constexpr uint8_t LDISASM_C(uint8_t* b) { return *b & 0xF; };

	constexpr uint8_t LEGACY_PREFIXES[] = { 0xF0, 0xF2, 0xF3, 0x2E, 0x36, 0x3E, 0x26, 0x64, 0x65, 0x66, 0x67 };
	constexpr uint8_t OP1_MOD_RM[] = { 0x62, 0x63, 0x69, 0x6B, 0xC0, 0xC1, 0xC4, 0xC5, 0xC6, 0xC7, 0xD0, 0xD1, 0xD2, 0xD3, 0xF6, 0xF7, 0xFE, 0xFF };
	constexpr uint8_t OP1_IMM8[] = { 0x6A, 0x6B, 0x80, 0x82, 0x83, 0xA8, 0xC0, 0xC1, 0xC6, 0xCD, 0xD4, 0xD5, 0xEB };
	constexpr uint8_t OP1_IMM32[] = { 0x68, 0x69, 0x81, 0xA9, 0xC7, 0xE8, 0xE9 };
	constexpr uint8_t OP2_MOD_RM[] = { 0x0D, 0xA3, 0xA4, 0xA5, 0xAB, 0xAC, 0xAD, 0xAE, 0xAF };
	...
	inline size_t get_instruction_length(const void* const address, const DisassmeblyMode mode = DisassmeblyMode::X86_64)
	{
		size_t offset = 0;
